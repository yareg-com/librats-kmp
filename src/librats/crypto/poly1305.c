...
#define POLY1305_64BIT 1
#else
#define POLY1305_32BIT 1
#endif
...
rats_poly1305_blocks(rats_poly1305_state_internal_t *st, const unsigned char *m, size_t bytes) {
    ...
    while (bytes >= rats_poly1305_block_size) {
        ...
        /* (partial) h %= p */
                      c = SHR(d0, 44); h0 = LO(d0) & 0xfffffffffffULL;
        ADDLO(d1, c); c = SHR(d1, 44); h1 = LO(d1) & 0xfffffffffffULL;
        ADDLO(d2, c); c = SHR(d2, 42); h2 = LO(d2) & 0x3ffffffffffULL;
